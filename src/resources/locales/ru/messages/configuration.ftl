...
command-configuration-welcome-setwelcome =
    ...
    В тексте можно использовать следующие параметры для замены:
    • <code>{"{user}"}</code> — для упоминания пользователя
    • <code>{"{user_name}"}</code> — для имени пользователя
    • <code>{"{user_id}"}</code> — для идентификатора пользователя
    • <code>{"{chat_title}"}</code> — для названия чата
    • <code>{"{timestamp}"}</code> — UNIX Timestamp

    Для форматирования текста используйте возможности в клиенте.
