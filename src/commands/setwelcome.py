...
async def command_setwelcome(message: Message) -> None:
    ...
    if len(msg) == 1:
        await message.reply(
            'Для установки нового приветственного сообщения напишите его через пробел после команды.\n\n'
    ...
    if msg[1] == 'disable':
        await message.reply(
            'Отправка приветственного сообщения и капча отключена для этого чата, для включения, установите новый текст или сбросьте до настроек по-умолчанию.')
        ...
        await database.setCaptchaText(message.chat.id,
